...
## 爆款三件套（本项目的灵魂，动代码前先读）
1. **纹样保底**：云纹/浪纹/漩涡三键一键出好图——随机是惊喜不是赌博
2. **成笺闭环**：覆纸拓印 → 宣纸+洒金+朱砂印章+唐诗笺文+编号（"全球仅此一张"）→ 保存 PNG
3. **双墨盘**：青绿（千里江山，主打）+ 松烟（水墨，二刷理由）
...
- 关键差异 vs Pavel：墨池深色底（艳色/淡墨都好看，容错率核心）→ 拓印时统一映射到暖宣纸底
...
- `js/patterns.js` — 纹样保底（程序编排的墨序）
- `js/rubbing.js` — 成笺渲染（Canvas 2D）
- `js/main.js` — 流程状态机
