...
## D12 · v1.5 提审定稿（2026-09-01，Maya 真机确认保存可用）
- 真机确认：容器内 blob 下载直存相册/文件可用 → 保存成功提示定稿「已保存 ✓」，失败才提示截图
- 包：`水影笺_小红书_v1.5.zip`（449KB）；资源引用版本号同步 1.5；提交入口：Builder Hub 上传 zip → 填名称/Slogan/图标 → 权限不勾 → 预览提审
- 上架后动作：发一篇挂载小工具的笔记（自检钩子链接可做演示素材），记录数据（449KB）
